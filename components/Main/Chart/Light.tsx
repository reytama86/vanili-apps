...
export default function Light() {
  ...
  const data7d = useMemo<DataPoint[]>(() => {
      ...
      for (let dayOffset = 6; dayOffset >= 0; dayOffset--) {
        const baseForDay = Math.floor(Math.random() * (15000 - 5000 + 1)) + 5000;
        ...
        times.forEach((hour) => {
          ...
          const variation = Math.floor(Math.random() * 10001) - 5000;
          const rawValue = baseForDay + variation;
          const value = Math.max(0, Math.min(20000, rawValue));
    ...
    const data1m = useMemo<DataPoint[]>(() => {
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        const sinus = Math.sin((dayIdx / 30) * Math.PI * 2) * 1000;
        const noise = (Math.random() - 0.5) * 1000;
        const raw = baseForDay + sinus + noise;
        const val = Math.max(5000, Math.min(20000, Math.round(raw)));
      ...
      <View style={styles.card}>
        ...
        <View style={styles.chartWrapper}>
          <LineChart
            ...
            color="#B4DC45"
            hideDataPoints
            yAxisLabelTexts={['0', '5000', '10000', '15000', '20000']}
              ...
              pointerLabelComponent: items => {
                ...
                return (
                  <View style={[styles.tooltip, { left: tooltipLeft, top: y - 55 }]}>
                    <Text style={styles.tooltipText}>{formatRibuan(value)} Lux</Text>
